...
int solver::run(int print_level, bool relax, double tol, double mipgap, const string& lin_solver, const string& mehrotra){
    int return_status = -1;
    bool violated_constraints = true, optimal = true;
    while(violated_constraints && optimal){
        if (_stype==ipopt) {
    #ifdef USE_IPOPT
            double mu_init = exp(-1)/exp(7);
//            double mu_init = 0.5;
            SmartPtr<IpoptApplication> iapp = IpoptApplicationFactory();
            iapp->RethrowNonIpoptException(true);
            ApplicationReturnStatus status = iapp->Initialize();
...
//            if (false) {
                ...
                iapp->Options()->SetNumericValue("warm_start_bound_push", 1e-6);
                iapp->Options()->SetNumericValue("warm_start_bound_frac", 1e-6);
                iapp->Options()->SetNumericValue("warm_start_slack_bound_frac", 1e-6);
                iapp->Options()->SetNumericValue("warm_start_slack_bound_push", 1e-6);
                iapp->Options()->SetNumericValue("warm_start_mult_bound_push", 1e-6);
            }
...
//                return 150;
    ...
    //        bonmin.initialize(prog.bonmin_prog);
        ...
        if (optimal) {
            violated_constraints = _model->has_violated_constraints(1e-4);
...
void run_models(const vector<shared_ptr<Model>>& models, int start, int end, SolverType stype, double tol){
    for (auto i = start; i<end; i++) {
        solver(*(models.at(i)),stype).run(0, false, tol, 0.01);
    }
}


void run_parallel(const vector<shared_ptr<Model>>& models, SolverType stype, double tol, unsigned nr_threads){
    vector<thread> threads;
    vector<bool> feasible;
    /* Split models into nr_threads parts */
    vector<int> limits = bounds(nr_threads, models.size());
