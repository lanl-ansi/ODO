...
class Switch: public gravity::aux{
public:
    
    
    string _type_name="SwitchGear";
    /** @brief Can be operated remotely */
    bool _remote = false;
    /** @brief Is used for protection */
    bool _protect = false;
    /** @brief Capital Cost. */
    double _capcost = 0;
    
    Switch(){};
    Switch(string name, double capcost, bool remote, bool protect): _capcost(capcost), _remote(remote), _protect(protect){
        _active = true;
        _name = name;
    };
    
    void print() const{
        cout << "name = " << _name << ", capital cost = " << _capcost << ", switch type = ";
        if(_remote){
             cout << "remote";
        }
        else {
...
public:
    ...
    /** @brief Indicates the minimum capacity of PV (solar) generation that need to be installed on current bus*/
    double _min_PV_cap = 0;
    
    /** @brief Indicates the maximum capacity of PV (solar) generation that need to be installed on current bus*/
    double _max_PV_cap = 0;
    
    /** @brief Indicates the minimum capacity of Wind generation that need to be installed on current bus*/
    double _min_Wind_cap = 0;
    
    /** @brief Indicates the maximum capacity of Wind generation that need to be installed on current bus*/
    double _max_Wind_cap = 0;
    ...
    /** @brief Indicates the capacity of existing PV (solar) generation on current bus */
    double _existing_PV_cap = 0;
    
    /** @brief Indicates the capacity of existing Wind generation on current bus */
    double _existing_Wind_cap = 0;
    ...
    /** @brief Corresponding set of wind gens (potential)  */
    std::vector<WindGen*> _pot_wind;
   ...
   */
    ...
    /** @brief Initialiser with Bus id and conductor properties
     @note By default, the network is in one phase, thus exactly one conductor is added to the bus when created. In a three phase network, this array will contain three conductors.
     */
    Bus(std::string name, double pl=0, double ql=0, double bs=0, double gs=0, double v_min=0, double v_max=0, double kvb=0,  int phase=3);
