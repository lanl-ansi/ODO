...
Bus::Bus(string name, double pl, double ql, double gs, double bs, double v_min, double v_max, double kvb, int nb_phases): Node(name), _kvb(kvb), _has_gen(false), vs(1){
    _cond.resize(nb_phases);
    for(auto i = 0; i<nb_phases;i++){
        Conductor* c = new Conductor(this, pl, ql, gs, bs, i);
        _cond[i] = c;
    }
    vbound.min = v_min;
    vbound.max = v_max;
...
//    }
    ...
    if(_type==3){
        printf("\tGrid connection Bus, infinite generator\n");
    }
    if(!_gen.empty() && _type!=3){
        printf("\tList of installed + potential diesel generators:\n");
        for(Gen * g:_gen) {
            g->print();
        }
    }
